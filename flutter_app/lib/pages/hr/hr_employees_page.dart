...
// ─────────────────────────────────────────────────────────────────────────────
// HR Employees Page — mirrors HRMS Lite Employees.jsx
// API: GET /api/users?role=employee  |  DELETE /api/users/:id
// ─────────────────────────────────────────────────────────────────────────────
...
class _HrEmployeesPageState extends State<HrEmployeesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_searchCtrl.text.trim().isNotEmpty) qs.add('search=${_searchCtrl.text.trim()}');
  ...
  Future<void> _confirmDelete(dynamic emp) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Are you sure you want to remove ${emp['name']}? This cannot be undone.'),
        actions: [
    ...
    return NavWrapper(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('Employees (${_employees.length})', style: theme.titleMedium),
          actions: [
            IconButton(
        ...
        body: Column(children: [
          ...
          Padding(
            ...
            child: TextField(
              ...
              onSubmitted: (_) => _load(),
              onChanged:   (_) { if (_searchCtrl.text.isEmpty) _load(); },
          ...
          if (_departments.length > 1)
            SizedBox(height: 44, child: ListView(
              ...
              children: _departments.map((d) => Padding(
                padding: const EdgeInsets.only(right: 8),
                child: ChoiceChip(
                  label: Text(d.isEmpty ? 'All' : d),
                  selected: _dept == d,
                  onSelected: (_) { setState(() => _dept = d); _load(); },
                  selectedColor: theme.primary,
                  labelStyle: TextStyle(
                    color: _dept == d ? Colors.white : theme.primaryText,
          ...
          Expanded(child: _loading
              ...
              : _error != null
                  ...
                  : _employees.isEmpty
                      ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Icon(Icons.people_outline, size: 64, color: theme.secondaryText),
                          const SizedBox(height: 12),
                          Text('No employees found', style: theme.labelMedium),
                        ]))
                      : RefreshIndicator(
                          onRefresh: _load,
                          child: ListView.builder(
                            ...
                            itemBuilder: (_, i) => _employeeTile(theme, _employees[i]),
                          ),
                        )),
        ]),
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          CircleAvatar(
            ...
            child: photoUrl.isEmpty
                ? Text(name.isNotEmpty ? name[0].toUpperCase() : '?',
                    style: TextStyle(color: t.primary, fontWeight: FontWeight.w700, fontSize: 16))
                : null,
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(name, style: t.bodyMedium.copyWith(fontWeight: FontWeight.w700))),
            ...
            Row(children: [
              Icon(Icons.business_outlined, size: 12, color: t.secondaryText),
              const SizedBox(width: 4),
              Text(dept, style: t.labelMedium.copyWith(fontSize: 11)),
              if (empId.isNotEmpty) ...[
                const SizedBox(width: 12),
                Text(empId, style: t.labelMedium.copyWith(fontSize: 11, color: t.primary, fontWeight: FontWeight.w600)),
              ],
            ]),
          ])),
          // Actions
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [
              ...
              const PopupMenuItem(value: 'attend', child: Text('Mark Attendance')),
