...
// ─────────────────────────────────────────────────────────────────────────────
// AMS Page — Full Attendance Management System (industry-grade)
// API: GET /api/users?role=employee  |  POST /api/attendance
//      GET /api/attendance/today
// Status enum: present | absent | late | half_day | work_from_home | on_leave
// ─────────────────────────────────────────────────────────────────────────────
...
class _AmsPageWidgetState extends State<AmsPageWidget>
  ...
  final Map<String, String> _pending = {};   // userId → status (unsaved changes)
  bool           _saving        = false;
  ...
  Future<void> _loadEmployees() async {
    ...
    try {
      ...
      setState(() => _employees = res['users'] ?? []);
    } finally { setState(() => _loadingEmp = false); }
  }
  ...
  Future<void> _loadToday() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Pre-fill pending from already-marked records
        for (final r in records) {
          final empId = r['employee']?['_id'] as String? ?? r['employee'] as String? ?? '';
          if (empId.isNotEmpty) _pending[empId] = r['status'] as String? ?? '';
        }
      });
    } finally { setState(() => _loadingToday = false); }
  }
  ...
  Future<void> _saveAll() async {
    ...
    for (final entry in _pending.entries) {
      ...
      try {
        ...
        // 409 = already marked for today → try PUT (update if exists)
        if (e.statusCode == 409) {
          try {
            ...
            if (existing != null) {
              await _api.put('/attendance/${existing['_id']}', {'status': entry.value});
              saved++;
            }
          } catch (_) { failed++; }
        } else {
            ...
            if (_pending.isNotEmpty)
              Padding(
                ...
                child: FilledButton.icon(
                  ...
                  label: Text('Save (${_pending.values.where((v) => v.isNotEmpty).length})'),
                ),
              ),
          ],
          bottom: TabBar(
            ...
            tabs: const [Tab(text: 'Mark Attendance'), Tab(text: 'Today\'s Report')],
          ),
        ),
    ...
    return Column(children: [
      ...
      GestureDetector(
        onTap: () async {
          ...
          if (d != null) {
            setState(() { _date = d.toIso8601String().substring(0, 10); _pending.clear(); });
            _loadToday();
          }
        },
        child: Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text('Marking for: $_date', style: t.bodyMedium.copyWith(color: t.primary, fontWeight: FontWeight.w600)),
      ...
      Padding(
        ...
        child: Wrap(spacing: 6, runSpacing: 4, children: _kStatuses.map((s) {
      ...
      // Employee list
      Expanded(child: _loadingEmp
          ? const Center(child: CircularProgressIndicator())
          : _employees.isEmpty
              ? Center(child: Text('No active employees found', style: t.labelMedium))
              : ListView.builder(
  ...
  Widget _employeeAttendRow(FlutterFlowTheme t, dynamic emp, String empId, String currentStatus) {
    ...
    return Container(
      ...
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: t.secondaryBackground, borderRadius: BorderRadius.circular(12)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (dept.isNotEmpty) Text(dept, style: t.labelMedium.copyWith(fontSize: 11)),
          ])),
          // Quick buttons
          Row(mainAxisSize: MainAxisSize.min, children: [
            _quickBtn(t, '✓', Colors.green, currentStatus == 'present', () => _mark(empId, 'present')),
            const SizedBox(width: 4),
            _quickBtn(t, '✗', Colors.red, currentStatus == 'absent', () => _mark(empId, 'absent')),
          ]),
        ]),
        const SizedBox(height: 8),
        // Full status strip
        SingleChildScrollView(
          ...
          child: Row(children: _kStatuses.map((s) {
  ...
  void _mark(String empId, String status) {
    setState(() {
      if (_pending[empId] == status) {
        _pending.remove(empId);  // toggle off
      } else {
  ...
  Widget _quickBtn(FlutterFlowTheme t, String label, Color color, bool active, VoidCallback onTap) =>
      GestureDetector(
        onTap: onTap,
        child: AnimatedContainer(
          ...
          child: Center(child: Text(label,
            style: TextStyle(color: active ? Colors.white : color, fontWeight: FontWeight.w800, fontSize: 14))),
        ),
      );
  ...
  Widget _buildReportTab(FlutterFlowTheme t) {
    ...
    for (final (k, _, __) in _kStatuses) counts[k] = 0;
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('$marked/$total marked', style: t.labelMedium.copyWith(color: t.primary)),
          ]),
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...
            if (unmarked > 0) _summaryChip(t, 'Unmarked', unmarked, Colors.grey),
          ]),
        ]),
      ),
      // Records list
      Expanded(child: _todayRecords.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.event_note_outlined, size: 64, color: t.secondaryText),
              const SizedBox(height: 12),
              Text('No attendance marked yet today', style: t.labelMedium),
            ]))
          : RefreshIndicator(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  final color  = _kStatuses.firstWhere((s) => s.$1 == status, orElse: () => ('', '', Colors.grey)).$3;
                  return Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(emp['name'] ?? '', style: t.bodyMedium.copyWith(fontWeight: FontWeight.w600)),
                        Text(emp['department'] ?? emp['employeeId'] ?? '', style: t.labelMedium),
                      ])),
                      Container(
  ...
  Widget _summaryChip(FlutterFlowTheme t, String label, int count, Color color) => Container(
