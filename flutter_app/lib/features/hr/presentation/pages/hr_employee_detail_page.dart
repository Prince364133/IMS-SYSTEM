...
class _HrEmployeeDetailPageState extends State<HrEmployeeDetailPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final [userRes, attRes] = await Future.wait([
        ...
        _api.get(
            '/attendance/employee/${widget.userId}?month=$_selectedMonth&limit=31'),
      ]);
      if (mounted) {
        setState(() {
          _user = userRes['user'];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(_user?['name'] ?? 'Employee Profile',
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _fieldGrid(t, [
          ...
          (
            'Salary',
            salary == 0 ? 'N/A' : '₹${salary.toStringAsFixed(0)}',
            Icons.currency_rupee_outlined
          ),
          ('Leave Balance', '$leavebal days', Icons.event_available_outlined),
          (
            'Join Date',
            joinDate != null ? joinDate.substring(0, 10) : 'N/A',
  ...
  Widget _fieldGrid(
      FlutterFlowTheme t, List<(String, String, IconData)> fields) {
    return Wrap(
        spacing: 16,
        runSpacing: 12,
                ...
                .map((e) => Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text(e.key.replaceAll('_', ' '),
                            style: TextStyle(
                                color: _statusColor(e.key), fontSize: 11)),
                      ]),
                    ))
                .toList()),
      ]),
