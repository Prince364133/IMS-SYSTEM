...
function Avatar({ user, size = 'md', emoji }: { user?: any; size?: 'sm' | 'md' | 'lg'; emoji?: string }) {
    ...
    async function handleCreate() {
        ...
        try {
            const payload = mode === 'dm'
                ? { memberId: selected[0] }
                : { isGroup: true, name: groupName.trim(), description: groupDesc, avatar: groupEmoji, memberIds: selected };
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    {isAdminOrHR && (
                        <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
                            <button onClick={() => { setMode('dm'); setSelected([]); }} className={clsx('flex-1 py-2 text-sm font-medium rounded-lg transition-all', mode === 'dm' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
                                💬 Direct Message
                            </button>
                            <button onClick={() => { setMode('group'); setSelected([]); }} className={clsx('flex-1 py-2 text-sm font-medium rounded-lg transition-all', mode === 'group' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
                                👥 Create Group
                            </button>
                        </div>
                    )}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {EMOJIS.map(e => (
                                        <button key={e} type="button" onClick={() => setGroupEmoji(e)}
                                            className={clsx('w-9 h-9 rounded-xl text-xl flex items-center justify-center border-2 transition-all', groupEmoji === e ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-gray-300')}>
                                            {e}
                                        </button>
                                    ))}
                                </div>
        ...
        <div className="w-72 border-l border-gray-100 flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="text-center">
                    <div className="text-4xl mb-2">{chat.avatar || '💬'}</div>
                    <p className="font-bold text-gray-900">{chat.name}</p>
                    {chat.description && <p className="text-xs text-gray-400 mt-1">{chat.description}</p>}
                    <p className="text-xs text-gray-400 mt-1">{chat.members.length} members</p>
                </div>
...
function ChatPageContent() {
    ...
    const handleTyping = (e: React.ChangeEvent<HTMLInputElement> | string) => {
        ...
        const lastWordMatch = textBeforeCursor.match(/@(\w[\w\s]*)$/);
    ...
    function insertMention(u: User) {
        ...
        const newTextBefore = textBeforeCursor.replace(/@(\w[\w\s]*)$/, `@${u.name} `);
    ...
    const mentionSuggestions = activeChat?.members.filter(m =>
        m._id !== user?._id && m.name.toLowerCase().startsWith(mentionQuery.toLowerCase())
    ) || [];
                        ...
                        return (
                            <button key={chat._id} onClick={() => selectChat(chat)}
                                ...
                                <div className="relative flex-shrink-0">
                                    {chat.isGroup ? <Avatar emoji={chat.avatar} /> : <Avatar user={other} />}
                                    {!chat.isGroup && (
            ...
            {activeChat ? (
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-3">
                        <div className="relative">
                            {activeChat.isGroup ? <Avatar emoji={activeChat.avatar} size="lg" /> : <Avatar user={getChatOther(activeChat)} size="lg" />}
                            {!activeChat.isGroup && (
